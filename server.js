...
app.delete('/api/quotes/:id', async (req,res,next)=>{try{const tid=tenantId(req);const ref=col(tid,'quotes').doc(req.params.id);const snap=await ref.get();if(!snap.exists)return res.status(404).json({error:'Cotización no encontrada'});await ref.set({deleted:true,deletedAt:now(),updatedAt:now()},{merge:true});res.json({ok:true,id:req.params.id});}catch(e){next(e)}});
